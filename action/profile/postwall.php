...
$profileuser = $_REQUEST['userprofile'];
$profileid = $_REQUEST['userprofileid'];
$t_site = $CONFIG->site;
...
$video = getVideo($message,'400','265');
...
// TODO: Image support on wall posting
/*
...
$now = time();		
ilog("Inserting new Post from $user_name in GROUP $group. MSG: $message");

$sql = "INSERT INTO `feed` (`group_id`, `user_id`, `user_name`,`user_full_name`, `user_profile_url`,"; 
$sql.= "`user_pic_url`, `message`, `tags`, `post_url`, `post_pic_url`, `post_type`, `link_src`,"; 
$sql.= "`link_title`, `link_description`, `link_video_src`, `source`, `date`, `parent`,"; 
$sql.= "`continent_code`, `country_code`, `country`, `region`, `city`, `longitude`, `latitude`)"; 
$sql.= "VALUES ('$group', $user_id, '$user_login_id','$user_name', '$profile_url', '$pic_url', '".db_escape($message)."', '$tags', '$post_url',"; 
$sql.= "'$post_pic_url', '$post_type', '$link_src', '$link_title', '".db_escape($link_desc)."', '$link_video', '$source',";
$sql.= "'$now', '-1',"; 
$sql.= " '$continent', '$countrycode', '$country', '$region', '$city',"; 
...
$sql = "INSERT INTO wall (post_id,user_id,scope) VALUES ($post_id,$profileid,1)";
get_query($sql);
...
include($CONFIG->home.'page/group/group_item.php');
...
*/
system_msg($babel->say('p_post_wall_success'));
header("Location: ".$CONFIG->site."/profile/$profileuser");
