...
if (isset($action) && $action != '')  {
  if ($action == 'action_join') {
    ...
    $message = '<hr style="height:1px;background-color:lightgray;width:98%;color:lightgray">'.$g->description;
    $message .= "<br><span style='margin-left:140px;'><img style='padding-top:10px' src='/template/default/_graphics/newgroup.jpg'></span>";
    $message .= '<hr style="height:1px;background-color:lightgray;width:98%;color:lightgray">';
...
<li id="item-<?=$post_id?>_<?=$view?>" data-timestamp='<?=$timestamp?>' class="rainbow-item <?=$newItem?>">
         ...
         <div class="rainbow-image-block rainbow-river-item clearfix">
      	  <div class="rainbow-image">
               <div class="rainbow-avatar rainbow-avatar-small">
                  ...
                  <a href="<?=$CONFIG->site?>/profile/<?=$user_login_id?>" class="">
                     <img onmouseout='onTriggerMouseOut(event,"avatar_helper")' onmouseover='loadProfile(this,"avatar_helper",357,0,"<?=$user_id?>","image")' src="<?=$CONFIG->site?>/template/default/images/spacer.gif" alt="<?=$user_name?>" title="<?=$user_name?>" class="" style="background: url(<?=$pic_url?>) no-repeat;"></a>
            ...
            <div class="rainbow-body">
               <div id='post-options-<?=$post_id?>' onmouseout='onPostOptionsOut(event,this)' class="post-options">
                  ...
                  <?php if (is_admin() || $me == $user_id) { ?>
                  <a onclick='editPost(<?=$post_id?>)'><?=$babel->say('p_edit')?></a><br>
                  <a onclick='deletePost(<?=$post_id?>)'><?=$babel->say('p_delete')?></a><br>
                  <?php } else {?>
                  <?=$babel->say('p_nopermission')?>
                  <?php } ?>
               ...
               <div class="rainbow-river-summary">
                  <a href="<?=$CONFIG->site?>/profile/<?=$user_login_id?>" class="rainbow-river-subject">
                  <b><span style='font-size:14px' onmouseout='onTriggerMouseOut(event,"avatar_helper")' onmouseover='loadProfile(this,"avatar_helper",356,-3,"<?=$user_id?>","link")'>
                  <?=$user_name?> 
                  </span></b></a> <?=$action_html?>

                  <?php if (is_admin() || $me == $user_id) { ?>
                  
                  <span onclick='showPostOptions(<?=$post_id?>)' style='position:relative;float:right;top:-0px'>
                  <div style='' class='rainbow-icon rainbow-icon-settings-alt'></div>
               ...
               <div id='itemcontent-<?=$post_id?>_<?=$view?>' class="feed-text-message">
                 	<?=$message?>
               </div>
      ...
      <div id="comments" class='rainbow-comment-box'>
          ...
          <?php if (!isset($group) || $group == '' || is_group_member($group)) { ?>
